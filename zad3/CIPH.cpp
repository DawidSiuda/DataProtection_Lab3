...
{
    const int numberOfCharacterInWord = 8;
...
{   /* Main program */
    ...
    std::string fileToEncodeName = "FileToEncrypt.txt";
    
    std::ifstream fileStream(fileToEncodeName);
    ...
    for (auto& word : vectorOfWords)
    {
        ...
        encodedText << BINTOHEXSTR(jotput);
    }

    std::cout << "FINAL TEXT: " << encodedText.str() << std::endl;
