   ...
   to infile and open the file for reading. */
{
    ...
    for (i = 0; i <= 15; i++) {
        ...
        for (j = 0; j <= 47; j++)
            kns[j][15 - i] = tkns[j];   // zmiana kolejnosci kluczy z i na 15-i
    }
...
std::string BINTOHEXSTR(gl64array    jotput)
...
std::vector<std::string> DIFSplitStringByNumber(std::string s)
{
    const int numberOfCharacterInWord = 16;
    const char* c = s.c_str();
    char buffer[numberOfCharacterInWord];
    std::vector<std::string> entries;
    entries.reserve(int(s.length() / numberOfCharacterInWord) + 1);

    int d = 0;
    for (int i = 0; i < s.length() - 1;)
    {
        if (d != numberOfCharacterInWord)
        {
            buffer[d] = c[i];
            d++;
            i++;
        ...
        {
            entries.push_back(std::string(buffer, numberOfCharacterInWord));

            //Clear array
            memset(buffer, 0, numberOfCharacterInWord);
            d = 0;
        }
    }
    entries.push_back(std::string(buffer, numberOfCharacterInWord));
...
{   /* Main program */
    ...
    std::cout << "SET 8 character password: " << std::endl;
    std::cout << "00000000\r";
    std::cin >> password;

    std::vector<std::string> vectorOf8chracterStrings;

    std::ifstream fileStream(".\\..\\..\\Lab4_3_encoder\\Lab4_3_encoder\\Encrypted.txt");
    ...
    while (fileStream)
    {
        ...
        stringBuilder << readChar;
    }

    std::string textFromFile = stringBuilder.str();
    std::cout << "Text from file: " << textFromFile << std::endl;

    auto vectorOfWords{ DIFSplitStringByNumber(textFromFile) };
    ...
    for (auto& word : vectorOfWords)
    {
        ...
        BINTOHEX(jotput);
        encodedText << BINTOTEXT(jotput);
    }


    std::cout << "DECRYPTED TEXT: " << encodedText.str() << std::endl;

    std::ofstream out("DecryptedText.txt");
    out << encodedText.str();
    out.close();
    return 0;
}
